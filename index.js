...
//   const specificJoke = jokes[jokeId-1];
...
app.put("/jokes/:id", (req,res) => {
  const id = parseInt(req.params.id);
  const replacememtJoke = {
    id: id,
    jokeText: req.body.text,
    jokeType: req.body.type,
  };
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  jokes[searchIndex] = replacememtJoke;
  console.log(jokes[searchIndex]);
  res.json(replacememtJoke);
})
...
app.delete("/all", (req,res) => {
  ...
  if (userKey === masterKey) {
    ...
    res
    .status(404)
    .json({ error: "You are no authorised to perform this action"});
  }
});
//had to call key a "key" in postman in order for it to work;
